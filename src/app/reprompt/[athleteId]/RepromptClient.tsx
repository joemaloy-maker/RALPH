...
import { FeedbackSummary } from '@/lib/feedback-aggregator'

interface RepromptClientProps {
  prompt: string
  athleteId: string
...
export function RepromptClient({
  prompt,
  athleteId,
  feedbackSummary,
  currentWeekNumber,
  ...
  const handleSubmitPlan = () => {
    ...
    // Store plan and context in sessionStorage, redirect to validate
    sessionStorage.setItem('pendingPlan', planText)
    sessionStorage.setItem('repromptContext', JSON.stringify({
      isReprompt: true,
      currentWeekNumber,
    }))
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            Time to build your next two weeks
          </h1>
          <p className="text-gray-600 dark:text-gray-400">
            Based on your recent training, here&apos;s what we know.
          </p>
        </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 mb-8">
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="col-span-2 text-center pb-4 border-b border-gray-200 dark:border-gray-800">
              <div className="text-5xl font-bold text-gray-900 dark:text-white mb-1">
                {feedbackSummary.completion_rate}%
              </div>
            ...
            <div>
              <div className="text-2xl font-semibold text-gray-900 dark:text-white">
                {feedbackSummary.completed + feedbackSummary.modified}
                <span className="text-gray-400 dark:text-gray-600">
                  /{feedbackSummary.total_sessions}
                </span>
              </div>
              <div className="text-sm text-gray-500 dark:text-gray-400">
                Sessions Completed
            ...
            {/* RPE */}
            {feedbackSummary.rpe_averages.overall !== null && (
              <div>
                <div className="text-2xl font-semibold text-gray-900 dark:text-white">
                  {feedbackSummary.rpe_averages.overall}
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  Avg RPE
                </div>
            ...
            {/* Skip patterns */}
            {feedbackSummary.skipped > 0 && (
              <div className="col-span-2 pt-4 border-t border-gray-200 dark:border-gray-800">
                <div className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                  Skip Patterns
                </div>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(feedbackSummary.skip_reasons)
                    .filter(([_, count]) => count > 0)
                    .map(([reason, count]) => (
                      <span
                        key={reason}
                        className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300"
                      >
            ...
            {/* Notes summary */}
            {feedbackSummary.notes.length > 0 && (
              <div className="col-span-2 pt-4 border-t border-gray-200 dark:border-gray-800">
                <div className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                  Your Notes
                </div>
                <ul className="space-y-1">
                  {feedbackSummary.notes.slice(0, 3).map((note, i) => (
                    <li
                      key={i}
                      className="text-sm text-gray-700 dark:text-gray-300 italic"
                    >
                      &ldquo;{note.length > 80 ? `${note.substring(0, 80)}...` : note}&rdquo;
                    </li>
                  ))}
                  {feedbackSummary.notes.length > 3 && (
                    <li className="text-xs text-gray-400">
                      +{feedbackSummary.notes.length - 3} more
                    </li>
                  )}
                </ul>
              </div>
        ...
        <div className="space-y-4 mb-8">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
            Here&apos;s what you&apos;ll tell the AI:
          </h2>
