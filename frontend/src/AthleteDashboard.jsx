...
const DASHBOARD_DATA = {
  ...
  today_session: {
    ...
    mindset: {
      ...
      elite_reframe: "The discomfort tells me I'm working hard—exactly what I came to do.",
      ...
      full: {
        ...
        attention_strategy: `• This interval is the ONLY interval
• Don't count remaining—that creates dread  
• When it ends, the next becomes the only one
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">— {mindset.source}</div>
            </div>
          </div>
      ...
      {expanded && (
        <div className="mt-3 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 space-y-4">
          ...
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
            <div className="text-xs text-emerald-400 uppercase mb-1">✓ Success Indicator</div>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {session.is_key_session && (
              <span className="px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-xs">⭐ KEY</span>
            )}
          </div>
        ...
        <div className="flex gap-2 mt-3 text-xs">
          {session.if_recovery_yellow && (
            <div className="flex-1 p-2 rounded-lg bg-amber-500/10 border border-amber-500/30">
              <span className="text-amber-400">🟡 Yellow:</span> <span className="text-slate-300">{session.if_recovery_yellow}</span>
            </div>
          )}
          {session.if_recovery_red && (
            <div className="flex-1 p-2 rounded-lg bg-rose-500/10 border border-rose-500/30">
              <span className="text-rose-400">🔴 Red:</span> <span className="text-slate-300">{session.if_recovery_red}</span>
            </div>
          )}
        </div>
      )}
    ...
    <div className="space-y-1">
      {sessions.map((day, i) => (
        <div key={i} className={`flex items-center gap-2 p-2 rounded-lg ${day.is_today ? 'bg-slate-800 ring-1 ring-cyan-500/50' : ''}`}>
          ...
          <span className={`text-xs ${day.is_done ? 'text-emerald-400' : 'text-slate-600'}`}>{day.is_done ? '✓' : '○'}</span>
        </div>
      ))}
    </div>
  </GlowCard>
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-cyan-400">{data.gap_analysis.ftp_current}→{data.gap_analysis.ftp_target}W</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-emerald-400">{formatPace(data.gap_analysis.pace_current)}→{formatPace(data.gap_analysis.pace_target)}</span>
                </div>
                <div className="h-1.5 bg-slate-800 rounded-full">
          ...
          {/* Load & Recovery */}
          <GlowCard className="p-4">
            <h3 className="text-sm font-semibold text-white mb-3">⚡ Form</h3>
